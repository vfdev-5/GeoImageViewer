...
def writeVectorFromMask(image, filename, layerName="Layer_name", layerFormat="ESRI Shapefile", geoTransformer = None):
        """
        ...
        - geoTransformer is a function ( gpt = geoTransformer(pt) )pixel to geographic point transformer which is used if non None to create geometry points.
          Output vector file will have WGS84 srs.
        """
        ImageCommon.assertOneBand(image)
        ...
        srs = None
        if geoTransformer is not None:
            srs = osr.SpatialReference(osr.SRS_WKT_WGS84)

        ogrLayer = outVectorDS.CreateLayer(layerName, srs, ogr.wkbPolygon)
        assert ogrLayer is not None, Global.logPrint("Failed to create layer in the vector file : " + filename, 'error')
...
def writeDataInGeoImage(filename, numpyArrayData, originAndStep=[[0.0,0.0],[1.0,-1.0]], metadata={}):
    """
        ...
        numpyArrayData - input data is given as numpy.array and should be in the following form:
            numpyArrayData.shape = (height, width, nbBands)
    ...
    assert isinstance(metadata, dict), Global.logPrint("metadata argument should be a dictionary", 'error')
    ...
    nbBands = numpyArrayData.shape[2]
    ...
    _setMetadata(dstDataset, metadata)

    for bandIndex in range(1,nbBands+1):
        bandData = numpyArrayData[:,:,bandIndex-1]
...
def _setMetadata(dstDataset, metadata):
    dstDataset.SetMetadata(metadata)
...
##
### Test 3 bands image:
##    data = numpy.zeros((100, 100, 3), dtype=numpy.int16)
