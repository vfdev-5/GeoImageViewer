...
def displayImage(image0, showMinMax=True, name="image", waitKey=True, rescale=True):
    ...
    for i in range(3):
        ...
        try:
            j = bandIndices.index(bandIndex)
            outputImage[:,:,i] = outputImage[:,:,j]
        except ValueError:
            bandIndices.append(bandIndex)
            ...
            if showMinMax:
                Global.logPrint("Band " + str(bandIndex) + " : min/max : " + str(minValues[bandIndex]) + ", " + str(maxValues[bandIndex]) )
                Global.logPrint("Band " + str(bandIndex) + " : min/max using mean/std: " + str(nmin) + ", " + str(nmax) )

            data = bandData.copy()
            data[bandData > nmax] = nmax
            data[bandData < nmin] = nmin
            if nmax > nmin:
                outputImage[:,:,i] = (255.0 * (data[:,:] - nmin) / (nmax - nmin))
...
def chooseRoi(image, returnImageArray=True):
    """
    Method to display whole image and choose a ROI
    Return [[offsetX,offsetY], roiImage] if returnImageArray == True or [[offsetX,offsetY], [w,h] otherwise

    """
    ImageCommon.assertImage(image)

    print "Use mouse left button to draw a rectangle"
    print "Use 'enter' key to exit and save the choice"

    displayLimit = 800
    maxdim = max(image.shape[0], image.shape[1])
    if maxdim > displayLimit:
        factor = maxdim * 1.0 / displayLimit
        image1 = cv2.resize(image, None, fx=1.0/factor,fy=1.0/factor)
    ...
    def onmouse(event, x, y, flags, params):
        ...
        if not (r[0] == r[1] == r[2] == r[3] == 0):
            cv2.rectangle(image2, (r[0],r[1]), (r[2],r[3]), (0,0,255))
        cv2.imshow("Choose ROI on the image", image3)

    cv2.destroyAllWindows()
    cv2.namedWindow("Choose ROI on the image")

    image2 = displayImage(image1, False, "Choose ROI on the image", False, False)

    while(1):
        image3 = image2.copy()
        cv2.setMouseCallback("Choose ROI on the image", onmouse, [isDrawing, r, image3])
    ...
    offsetX = int(r[0]*factor)
    offsetY = int(r[1]*factor)
    w = int(factor*(r[2]-r[0]))
    h = int(factor*(r[3]-r[1]))

    if returnImageArray:
        roiImage = image[offsetY:offsetY+h,offsetX:offsetX+w].copy()
        return [[offsetX,offsetY], roiImage]
    else:
...
if __name__ == "__main__":
    ...
    filenames.append("C:\\VFomin_folder\\PISE_project\\IRT\Data\\Projets_TPZ\\Gabon\\IMG\\WGS84_AOI_ASA_WSM_1PTDPA20090312_091022_000000732077_00136_36763_0410.N1.tif")

    image = loadImage(filenames[-1])
    [offset, roiImage] = chooseRoi(image)
    if offset is not None:
        print offset
        displayImage(roiImage, True, "Roi")
        cv2.destroyAllWindows()
